        ...
        <el-card class="box-card">
          ...
          <el-dialog :title="`添加POC代码: ${currentCveId2}`" :visible.sync="uploadDialogVisible" width="30%">
            <div>
              ...
              <el-upload ref="upload" action="" :file-list="fileList" :on-change="handleFileChange"
                :before-upload="beforeUpload" :limit="1" :auto-upload="false" :multiple="false">
...
export default {
  ...
  methods: {
    ...
    // 从API获取数据
    fetchFromApi() {
      this.$axios.post('api/pocSearch', { ip: this.currentIp })
        .then(response => {
