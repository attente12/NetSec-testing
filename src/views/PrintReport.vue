...
export default {
  ...
  methods: {
    ...
    async fetchBaselineDetails() {
      try {
        const response = await neoFetch(this.$store.state.fetchUrl + `/userinfo?ip=${this.assetData.ip}`,{headers: { 'content-type': 'application/json' }});
    ...
    async fetchClassifyDetails() {
      try {
        const response = await neoFetch(this.$store.state.fetchUrl + `/level3Userinfo?ip=${this.assetData.ip}`,{headers: { 'content-type': 'application/json' }});
        const data = await response.json();
        if (data && data.checkResults) {
